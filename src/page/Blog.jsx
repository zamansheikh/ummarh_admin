...
export default function Blog() {
	...
	useEffect(() => {
		const fetchData = async () => {
			setLoading(true);
			try {
				const [mainCitiesRes, locationsRes] = await Promise.all([
	...
	const saveBlog = async () => {
		...
		try {
			...
			setTitle('');
			setSelectedLocation('');
			setBlogData([...blogData, response.data]); // Update blog list
		} catch (error) {
							...
							<div className="flex flex-col gap-4 max-w-4xl mx-auto">
								...
								<div className="flex justify-end mt-4">
									<button
										...
										className={`px-6 py-2 bg-[#C9A038] text-white rounded-md ${saveLoading ? 'opacity-50 cursor-not-allowed' : ''
											}`}
									>
