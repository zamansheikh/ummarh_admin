...
export default function Ummrah() {
  ...
  const fetchBlog = async () => {
    try {
      const response = await axiosInstance.get("/mainapp/ummrah/");
  ...
  const handleDelete = async (id) => {
    try {
      const response = await axiosInstance.delete(`/mainapp/ummrah/${id}/`);
  ...
  const saveBlog = async () => {
    ...
    try {
      ...
      console.log("Ummrah Blog saved successfully:", response.data);
      alert("UmmrahBlog saved successfully!");
      fetchBlog()
      ...
      console.error("Error saving ummrah:", error);
      alert("Error saving ummrah.");
    }
  };
      ...
      <div className="pr-3">
        ...
        <div className="flex justify-start items-center gap-3 mb-4 border-b">
          <button
            ...
            className={activeTab === "upload" ? " text-[#C9A038] border-b-2 border-b-[#C9A038]" : ""}
          >
            Upload
          </button>
          <button
            onClick={() => setActive("list")}
            className={activeTab === "list" ? "text-[#C9A038] border-b-2 border-b-[#C9A038]" : ""}
          >
            List
          </button>
        </div>
          ...
          {activeTab === "upload" ? (
            <div className="flex flex-col gap-5 px-4 w-[60vw]">
              ...
              <div className="flex flex-col items-center">
                <label htmlFor="file-upload" className="cursor-pointer w-full h-[12vh] border border-gray-[rgba(34, 34, 34, 0.49)] rounded-md text-center flex flex-col items-center justify-center">
                  <FaUpload className="text-4xl text-gray-500 mb-2" /> {/* Upload icon */}
                  <span className="text-gray-500">Blog Image</span> {/* Label text */}
                  <input
                    id="file-upload"
                    type="file"
              ...
              <div className="mt-12 flex items-end justify-end mr-3">
                <button onClick={saveBlog} className="px-10 rounded-md py-2 bg-[#C9A038]">
                  Save
                </button>
              </div>
            </div>
          ) : (
            <UmmrahList fetchBlog={fetchBlog} data ={blogData} onDelete={handleDelete}/>
          )}
        </div>
      </div>
    </Sidebar>
