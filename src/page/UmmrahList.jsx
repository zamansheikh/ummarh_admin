...
export default function UmmrahList({ data = [], onDelete, fetchBlog }) {
	...
	const handleupdate = async () => {
		try {
			...
			if (response.status === 200) {
				setModalOpen(false);
				alert('Ummrah Updated Sucessfully');
				fetchBlog();
			}
		} catch (error) {
			console.error('Error fetching ummrah list:', error);
		}
	};
	...
	return (
		<div className="p-6">
			{/* Header */}
			<h2 className="text-2xl font-bold mb-4">Ummrah Blog List</h2>
								...
								<tr key={index} className="border-b">
									<td className="p-2 ">
										<img
											src={post.image}
											alt="Blog"
											className="w-[170px] h-[100px] bg-cover rounded"
										/>
									</td>
									<td className="p-2">{post.title}</td>
									<td
										className="p-2 max-h-[5rem]  overflow-hidden text-ellipsis"
										style={{
											display: '-webkit-box',
											WebkitLineClamp: 3,
